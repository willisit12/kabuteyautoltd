...
<section id="inventory" class="py-10 bg-background transition-colors duration-500 relative" x-data="inventoryManager()">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <!-- Inventory Grid -->
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10" id="car-grid">
            <?php foreach ($latestCars as $car):
                $image = $car['primary_image'] ?? 'https://via.placeholder.com/400x300?text=No+Image';
            ?>
            <div class="bg-card backdrop-blur-md rounded-[2.5rem] overflow-hidden border border-border/50 hover:border-accent/40 transition-all duration-700 group hover:-translate-y-3 shadow-sm hover:shadow-2xl">
                <div class="relative h-56 md:h-64 overflow-hidden">
                    <img src="<?php echo url($image); ?>" alt="<?php echo clean($car['make'] . ' ' . $car['model']); ?>" class="w-full h-full object-cover transition duration-1000 group-hover:scale-110">
                    <div class="absolute inset-0 bg-gradient-to-t from-background/90 via-transparent to-transparent opacity-80"></div>
                    <?php if ($car['featured']): ?>
                        <div class="absolute top-6 right-6 bg-accent text-white text-[9px] md:text-[10px] font-black px-3 py-1.5 rounded-full uppercase tracking-widest shadow-lg"><?php echo __('featured_element'); ?></div>
                    <?php endif; ?>
                </div>
                <div class="p-8 md:p-10 flex flex-col h-full">
                    ...
                    <a href="<?php echo url('car-detail/' . $car['slug']); ?>" class="btn-premium block w-full text-center bg-muted hover:bg-accent text-foreground hover:text-white py-5 rounded-2xl font-black uppercase tracking-tighter transition-all border border-border hover:border-accent text-sm shadow-sm group/btn">
                        Explore Specifications <i class="fas fa-arrow-right ml-2 text-accent group-hover/btn:text-white transition-colors"></i>
                    </a>
                </div>
            </div>
            <?php endforeach; ?>
